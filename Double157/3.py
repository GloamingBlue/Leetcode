...
class Solution:
    ...
    def assignEdgeWeights(edges: List[List[int]]) -> int:
        ...
        depth = [get_path_length(1, i) for i in range(1, n + 1)]
        return 2 ** (max(depth) - 1) % MOD


if __name__ == '__main__':
    print(f'{Solution.assignEdgeWeights([[1, 2]]) = }')
    print(f'{Solution.assignEdgeWeights([[1, 2], [1, 3], [3, 4], [3, 5]]) = }')
